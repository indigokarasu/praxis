...
Scans unevaluated skill journals for behavioral signals.
All 5 unevaluated journals are no-op/observation with no new signals.
"""
import json
import os
...
JOURNAL_DIR = "~/.hermes/commons/journals/ocas-praxis"
...
def extract_signals_from_dict(data, canonical):
    ...
    if isinstance(sources, dict) and sources:
        for src_name, src_val in sources.items():
            if isinstance(src_val, str):
                # String-value sources (scan-0600+)
    ...
    # Forge journal-scan: no-op detection
    result_val = data.get("result", "")
    if result_val in ("no_files_found", "nothing_to_process", "no-op"):
        if data.get("unprocessed_proposals", 0) == 0 and data.get("unprocessed_decisions", 0) == 0:
...
if truly_new:
    ...
    if remaining_proposals:
        ...
        if new_shifts:
            # Rewrite shifts file with all existing + new
            all_shifts_dict = {}
            for s in all_shifts:
