...
## Files Modified
- `~/ingest_main.py` — journal ingest with noise filtering, dedup, evidence-before-eval
- `~/lesson_extract.py` — lesson extraction with noise filter, phase validation, content dedup
- `~/shift_propose_v2.py` — shift proposal with priority-based cap enforcement
- `~/debrief_gen.py` — daily debrief generation