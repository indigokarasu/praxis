...
- 1 journal clean: `chronicle/run_cron_20260604_224411.json` (non-standard schema, no status field)
...
- Chronicle journals use a non-standard schema (no top-level `status`, uses `decision.journals_scanned` etc.) — the ingest script must handle missing fields gracefully