...
**Eval file path format:** ALWAYS use relative paths from `commons/journals/` (e.g., `ocas-forge/2026-06-26/forge-scan-TS.json`). NEVER use absolute paths (`<hermes-home>/profiles/indigo/commons/journals/...`). Absolute paths break the dispatcher's grep-based eval checks. Confirmed 2026-06-26 dispatch #154.
...
- **`journals_evaluated.jsonl` path**: check both `{profile}/commons/journals/ocas-praxis/` and `<hermes-home>/commons/journals/ocas-praxis/`.
...
- **Dual-dir listing double-scans when both paths resolve to the same directory (fixed 2026-09-25):** `JOURNALS_DIRS` lists both `~/.hermes/commons/journals` and `~/.hermes/profiles/<agent>/commons/journals`. When these are the same physical directory (same inode — true in this deployment), every not-yet-evaluated journal is discovered twice and gets duplicate eval entries (e.g. 2 unique journals → "Found 4 new journals", 4 duplicated eval lines; benchmark check: the same journal_id appearing twice in the eval tail). Fixed in `templates/dispatch_ingest_template.py` with an in-flight `pending_ids` dedup set — keep that guard when copying the template, and if a wave copy lacks it, expect doubled counts and duplicate eval lines (membership-based dedup makes them harmless, but counters inflate).