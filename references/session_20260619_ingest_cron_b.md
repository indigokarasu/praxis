...
=======
- **Stale script cleanup incident:** The cleanup pass deleted 46 files from the data directory root, including production scripts (`praxis_review_indigo.py`, `praxis_ingest_run.py`, `praxis_common.py`, `praxis_self_signaler.py`) that had been placed in the root rather than `scripts/`. Recovery: copied from skill directory `~/.hermes/profiles/indigo/skills/ocas-praxis/scripts/`.
>>>>>>> Stashed changes
